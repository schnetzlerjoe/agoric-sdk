 ...
 * @param {import('ava').Assertions} t
 */
async function testRemotePegPFM(t) {
  t.plan(28);
  ...
  // Pretend we're Agoric.
  const { pfmHandler: chandler, subscription: connectionSubscription } =
    await E(pegasus).makePegasusConnectionKit();
  const connP = E(portP).connect(portName, chandler);
...
test('remote peg with PFM', t => testRemotePegPFM(t));
