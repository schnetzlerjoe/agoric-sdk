 ...
 *   getArtifactNames: (includeHistorical: boolean) => Iterable<string>,
 * }} TranscriptStore
 *
 * @typedef {{
...
export function makeTranscriptStore(db, ensureTxn, noteExport) {
   ...
   *
   * @yields {string}
   * @returns {Iterable<string>}  An iterator over the names of all the artifacts requested
   */
  ...
  async function* exportSpan(vatID, startPos) {
    for (const entry of readSpan(vatID, startPos)) {
      const item = typeof entry === 'string' ? Buffer.from(entry) : entry;
      yield item;
      yield nl;
    }
  }
