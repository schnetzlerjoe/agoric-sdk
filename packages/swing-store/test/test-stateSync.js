...
test('export and import data for state sync', async t => {
  ...
  const kvData = Array.from(exporter.getKVData());
  t.deepEqual(kvData, [
  ...
  const artifactNamesAll = Array.from(exporter.getArtifactNames(true));
  ...
  const artifactNamesCurr = Array.from(exporter.getArtifactNames(false));
